...
class BaseAPIClient(ABC):
    ...
    @abstractmethod
    def search_tv_show(self, title: str, season: Optional[int] = None, episode: Optional[int] = None) -> Optional[Dict[str, Any]]:
        pass
...
class TMDBClient(BaseAPIClient):
    ...
    def search_tv_show(self, title: str, season: Optional[int] = None, episode: Optional[int] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if results:
                ...
                result = {
                    'title': show.get('name'),
                    'year': int(show.get('first_air_date', '0000')[:4]) if show.get('first_air_date') else None,
                    'tmdb_id': show.get('id'),
                    'season': season,
                    'episode': episode
                }
                ...
                if season and episode:
                    episode_info = self._get_episode_info(show.get('id'), season, episode)
                    if episode_info:
    ...
    def _get_episode_info(self, series_id: int, season: int, episode: int) -> Optional[Dict[str, Any]]:
        params = {
            'api_key': self.api_key,
            'language': 'en-US'
        }
        ...
        try:
            ...
            for result in results:
                ...
                if series:
                    ...
                    if season and episode:
                        episode_info = self._get_episode_info(series.get('id'), season, episode)
                        if episode_info:
        ...
        for client in [self.tvdb, self.tmdb]:
            if client:
                ...
                if result:
                    media_info.title = result.get('title', media_info.title)
                    media_info.year = result.get('year', media_info.year)
                    media_info.episode_title = result.get('episode_title', media_info.episode_title)
                    media_info.tmdb_id = result.get('tmdb_id', media_info.tmdb_id)
                    media_info.tvdb_id = result.get('tvdb_id', media_info.tvdb_id)
                    break
