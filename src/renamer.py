...
from .models import MediaInfo, RenameResult, MediaType
    ...
    def rename_file(self, media_info: MediaInfo) -> RenameResult:
        try:
            ...
            if self.config.dry_run:
                self.logger.info(f"DRY RUN: Would rename {media_info.original_path} -> {new_path}")
                return RenameResult(
                    original_path=media_info.original_path,
                    new_path=new_path,
                    success=True,
        ...
        for file_path in directory.rglob("*"):
            if file_path.is_file() and file_path.suffix.lower() in self.config.supported_extensions:
                ...
                api_manager = APIClientManager(
                    tmdb_key=self.config.tmdb_api_key,
                    tvdb_key=self.config.tvdb_api_key
                )
                ...
                if self.config.verbose:
                    if result.success:
                        self.logger.info(f"Renamed: {result.original_path} -> {result.new_path}")
                    else:
                        self.logger.error(f"Failed to rename {result.original_path}: {result.error}")
