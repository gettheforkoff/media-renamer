...
class TVShowGroup:
    ...
    year: Optional[int] = None
    tvdb_id: Optional[str] = None
    directories: List[TVShowDirectory] = field(default_factory=list)
...
class TVShowConsolidator:
    ...
    def _analyze_directory_for_tv_content(
        self, directory: Path
    ) -> Optional[TVShowDirectory]:
    ...
    def _extract_show_title(self, dir_name: str) -> str:
        ...
        cleaned = re.sub(
            r"\b(720p|1080p|480p|4K|WEB|BluRay|DVD|h264|x264|h265|x265)\b",
            "",
            cleaned,
            flags=re.IGNORECASE,
        )
    ...
    def _normalize_show_title(self, title: str) -> str:
        ...
        for base_name, variants in self.show_normalizations.items():
            if (
                ...
                or base_name in normalized
            ):
                return base_name

        return normalized

    def _group_directories_by_show(
        self, tv_directories: List[TVShowDirectory]
    ) -> List[TVShowGroup]:
        ...
        for i, tv_dir in enumerate(tv_directories):
            ...
            # Create new group
            group = TVShowGroup(
                show_title=tv_dir.show_title, year=tv_dir.year, directories=[tv_dir]
            )
    ...
    def _are_same_show(self, dir1: TVShowDirectory, dir2: TVShowDirectory) -> bool:
        ...
        similarity = SequenceMatcher(
            None, dir1.normalized_title, dir2.normalized_title
        ).ratio()
        if similarity > 0.8:
            return True
    ...
    def _enhance_groups_with_api_data(
        self, groups: List[TVShowGroup]
    ) -> List[TVShowGroup]:
        ...
        for group in groups:
            ...
            self.logger.info(
                f"Enhanced group: {group.show_title} ({group.year}) [tvdb-{group.tvdb_id}]"
            )
    ...
    def _consolidate_show_group(self, group: TVShowGroup, root_directory: Path) -> Dict:
        ...
        self.logger.info(
            f"Consolidating {len(group.directories)} directories into: {unified_name}"
        )
        ...
        for tv_dir in group.directories:
            ...
            if season_num:
                ...
                if self.config.dry_run:
                    self.logger.info(
                        f"DRY RUN: Would create season directory: {season_dir}"
                    )
                    self.logger.info(
                        f"DRY RUN: Would move contents from {tv_dir.path} to {season_dir}"
                    )
                else:
    ...
    def _map_year_to_season(
        self, year: Optional[int], group: TVShowGroup
    ) -> Optional[int]:
        ...
        try:
            for item in source_dir.iterdir():
                ...
                if item.is_file():
                    if dest_item.exists():
                        self.logger.warning(
                            f"File already exists, skipping: {dest_item}"
                        )
                        continue
            ...
            self.logger.error(
                f"Error moving contents from {source_dir} to {dest_dir}: {e}"
            )
