...
from media_renamer.tv_show_consolidator import TVShowConsolidator, TVShowDirectory, TVShowGroup
...
class TestTVShowConsolidator:
    ...
    def test_normalize_show_title(self, config):
        ...
        assert consolidator._normalize_show_title("Friday Night SmackDown") == "smackdown"
        
        # Test other shows
        assert consolidator._normalize_show_title("Supernatural") == "supernatural"
        assert consolidator._normalize_show_title("The Walking Dead") == "the walking dead"
    ...
    def test_extract_show_title(self, config):
        ...
        assert "SmackDown" in consolidator._extract_show_title("SmackDown.2012.Pack.720p.WEB.h264-WD")
        assert "SmackDown" in consolidator._extract_show_title("SmackDown 2018")
        
        # Test other shows
        assert "Supernatural" in consolidator._extract_show_title("Supernatural Season 1")
    ...
    def test_extract_year_from_name(self, config):
        ...
        assert consolidator._extract_year_from_name("SmackDown.2012.Pack.720p.WEB.h264-WD") == 2012
    ...
    def test_map_year_to_season(self, config):
        ...
        group = TVShowGroup(
            ...
            year=1999  # SmackDown started in 1999
        )
        ...
        assert consolidator._map_year_to_season(2018, group) == 20 # 20th season
    ...
    def test_generate_unified_directory_name(self, config):
        ...
        expected = "WWE SmackDown (1999) [tvdb-73255]"
        assert result == expected
        
        # Test without TVDB ID
        group_no_tvdb = TVShowGroup(
            show_title="WWE SmackDown", 
            year=1999
        )
    ...
    def test_discover_tv_directories(self, config, sample_tv_directories, mock_api_manager):
        with patch('media_renamer.tv_show_consolidator.MetadataExtractor'):
            ...
            with patch.object(consolidator, '_analyze_directory_for_tv_content') as mock_analyze:
                mock_analyze.side_effect = lambda d: TVShowDirectory(
                    path=d,
                    show_title="SmackDown" if "smackdown" in d.name.lower() else "Supernatural",
                    normalized_title="smackdown" if "smackdown" in d.name.lower() else "supernatural",
                    year=2018
                ) if d.name != "sample.mkv" else None
...
class TestTVShowConsolidatorIntegration:
    ...
    def test_full_consolidation_workflow(self, mock_api_manager_class, config, sample_tv_directories):
        ...
        with patch.object(consolidator, '_analyze_directory_for_tv_content') as mock_analyze:
            ...
            assert "WWE SmackDown (1999) [tvdb-73255]" in result["unified_directory"]
